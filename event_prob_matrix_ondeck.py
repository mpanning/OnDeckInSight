...
filename = "{}_{}_{:.1f}km_{:.3f}_{:.3f}_ondeck_nev.csv".format(model, snr,
                                                         depth_in_km, f1, f2)
...
filename = "{}_{}_{:.1f}km_{:.3f}_{:.3f}_ondeck_prob.csv".format(model, snr,
                                                          depth_in_km, f1, f2)
with open(filename, 'w') as csvfile:
    csvwriter = csv.writer(csvfile)
    csvwriter.writerow(np.append(['a values/b values'], bvals))
...
filename = "{}_{}_{:.1f}km_{:.3f}_{:.3f}_ondeck_m0total.csv".format(model, snr,
                                                             depth_in_km,
                                                             f1, f2)
with open(filename, 'w') as csvfile:
    csvwriter = csv.writer(csvfile)
    ...
    for ii in range(len(avals)):
        for jj in range(len(bvals)):
            ...
            plt.title("Threshold defined by model {} and snr {}".format(model,
                                                                        snr))
            fig.savefig("{}_{}_{}_{}_ondeck_probability.png".format(model, snr,
                                                             avals[ii],
                                                             bvals[jj]))
            plt.close(fig)
            ...
            fig.savefig("{}_{}_{}_{}_ondeck_mag_events.png".format(model, snr,
                                                            avals[ii],
                                                            bvals[jj]))
            plt.close(fig)
